...
public:
    ...
    void dfs(TreeNode* root, vector<vector<int>> &ans, vector<int> &path, int tmp, int sum) {
        ...
        path.push_back(root->val);
        if (root->left) 
            dfs(root->left, ans, path, tmp + root->val, sum);
        if (root->right) 
            dfs(root->right, ans, path, tmp + root->val, sum);
        path.pop_back();
    }
};
```
